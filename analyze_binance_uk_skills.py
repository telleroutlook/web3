...
def _filter_rows(rows: Iterable[Dict[str, str]]) -> List[Dict[str, str]]:
    matched: List[Dict[str, str]] = []
    for row in rows:
        if row["company"] != "Binance":
            continue
        if "Computer Science / Software Engineering" not in row["categories"]:
            continue
        countries = [c.strip() for c in row["location_countries"].split(";") if c.strip()]
...
def generate_report() -> None:
    ...
    if total == 0:
        ...
        lines.append("")
        lines.append("- Filtered `majors.csv` for records where `company == \"Binance\"`, `categories` contained \"Computer Science / Software Engineering\", and `location_countries` included \"United Kingdom\".")
